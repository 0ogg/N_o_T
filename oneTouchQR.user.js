...
const Utils = {
    ...
    makeDraggable: function(element, handle = null, onPositionChange = null, storageKey = null) {
        ...
        if (savedPosition) {
            ...
            } catch (e) { console.error("저장된 위치 정보를 불러오는 중 오류 발생:", e); }
        }
        ...
        const onMouseMove = (e) => {
            ...
            element.style.right = right + "px";
            element.style.bottom = bottom + "px";
            if (onPositionChange) { onPositionChange({ right, bottom }); }
    ...
    createElement: function(tag, attributes = {}, children = []) {
        ...
        Object.entries(attributes).forEach(([key, value]) => {
            ...
            if ((key === 'disabled' || key === 'checked' || key === 'open') && !value) { return; }
...
const Storage = {
    ...
    initializeCoreSettings: function() {
        const settingsToInitialize = {
            ...
            'imagePanelDimensions': () => ({ width: 800, height: 700 }),
            // [추가] '유틸' 카테고리의 기본 표시 설정을 추가하여, 첫 사용자에게 폴더 기능이 보이도록 함
            'remoteLayout': () => [{ id: '유틸', visible: true, icon: 'fa-solid fa-toolbox' }],
            ...
            'settingsPanelPosition': () => ({ right: 50, bottom: 50 }),
            'imagePanelPosition': () => ({ right: 50, bottom: 50 }),
        };
    ...
    getLorebooks: function() { return this.get('lorebookPresets', []); },
    setLorebooks: function(lorebooks) { this.set('lorebookPresets', lorebooks); },
    getLorebookById: function(id) { return this.getLorebooks().find(lb => lb.id === id); },

    getRemoteLayout: function() { return this.get('remoteLayout', []); },
    setRemoteLayout: function(layout) { this.set('remoteLayout', layout); },
    getRemoteFavorites: function() { return this.get('remoteFavorites', []); },
    setRemoteFavorites: function(favorites) { this.set('remoteFavorites', favorites); },

    // [추가] Firebase 로깅 관련 설정
    getFirebaseConfig: function() { return this.get('firebaseConfig', ''); },
    setFirebaseConfig: function(config) { this.set('firebaseConfig', config); },
    isLoggingEnabled: function() { return this.get('loggingEnabled', false); },
    setLoggingEnabled: function(isEnabled) { this.set('loggingEnabled', isEnabled); },
    ...
    getDefaultPromptsData: function() {
        return [
            { id: 'default-prompt-translate', name: 'DEFAULT 번역', content: '다음 텍스트를 자연스럽고 매끄러운 한국어로 번역하세요. 문맥과 뉘앙스를 반영하며, 현지화가 필요한 부분은 자연스럽게 조정하세요. 사설없이 본문 텍스트 데이터만 출력하세요.', category: 'DEFAULT' },
            { id: 'default-prompt-ko-en-prefix', name: 'DEFAULT 한영번역란', content: '다음 제공된 문장을, 첨부된 소설의 문맥에 자연스럽게 이어지도록 영어로 번역하세요. 직역을 피하고, 감정·상황·말투까지 반영해 자연스럽고 맥락에 맞는 번역을 하세요.\n\n번역할 문장:', category: 'DEFAULT' },
            { id: 'default-prompt-image-main', name: 'DEFAULT 삽화 프롬 생성', content: "( 삽화 삽입 지점: 바로 위 장면을 삽화 생성을 위한 프롬프트화 하세요.)\n\n### **역할 정의**\n당신은 제공된 소설 텍스트를 분석하여, 지정된 지점 `(삽화 삽입 지점)`에 삽입될 고품질 삽화 생성을 위한 프롬프트를 작성하는 AI 어시스턴트입니다. \n당신이 생성 해야 할 것은 소설 전체의 삽화 프롬프트가 아니라 `(삽화 삽입 지점)` 문구가 있는 부분에 들어갈 삽화의 프롬프트입니다. 앞의 장면은 맥락 정보를 제공하기 위한 컨텍스트이며, 삽화가 될 장면은 첨부된 텍스트의 **가장 후반부**임을 기억하세요.\n\n### **전체 출력 양식**\n최종 결과물은 아래의 양식을 반드시 준수해야 합니다. 프롬프트 태그 외에 어떠한 설명이나 문장 부호도 앞뒤에 추가하지 마세요.\n\n`메인 프롬프트 | 캐릭터1 정보 | 캐릭터2 정보 ...`\n\n---\n\n### **프롬프트 작성 지침**\n\n#### **1. 메인 프롬프트**\n삽화의 전체적인 배경과 분위기를 설정합니다. 다음 요소들을 조합하여 상세하고 풍부한 태그를 구성하세요.\n\n*   **필수 요소**: 장소나 배경(가장 중요), 가구, 아이템, 수위 (sfw/nsfw), 노출 여부, 체위/자세, 등장인물수, 상황 요약 문장\n*   **상황 요약 문장**: 그림 전체의 분위기와 핵심 상황을 요약하는 영어 문장을 추가하세요. (예: `a comedic fight between two girls in a dark factory`, `a couple having stealthy sex under a desk`)\n*   **구체적 자세 명칭**: `full nelson`, `princess carry`, `missionary position`과 같이 그림의 중심이 되는 구체적인 자세나 행위의 명칭을 태그로 추가하여 정확도를 높이세요.\n\n#### **2. 캐릭터 프롬프트**\n각 캐릭터의 정보는 `|`로 구분합니다. 양식은 다음과 같습니다.\n`타입, 이름 (원작), 현재 외모, 자연어로 된 정확한 행동 설명`\n\n*   **타입**: `girl`, `boy`, `other` 중 선택합니다.\n*   **이름 (원작)**:\n    *   **원작 캐릭터**: AI가 학습한 데이터를 활용할 수 있도록 정확한 `캐릭터 영문명 (작품 영문명)`을 기입하세요. (예: `gojou satoru (jujutsu kaisen)`) 컨텍스트 내에서 묘사되지 않은 외모 정보는 추가하지 마세요.\n    *   **창작 캐릭터 (OC)**: 잘못된 정보가 연결되는 것을 막기 위해 이름 대신 `original character` 태그만 사용하세요. 제공된 컨텍스트의 묘사를 바탕으로 머리색, 눈 색, 헤어스타일 등 외모 정보를 최대한 상세하게 작성해야 합니다.\n*   **현재 외모**: 소설 내용에 기반한 `표정`, `의상`, `자세`, `노출된 신체 부위`, `상처나 찢어진 옷` 등의 시각적 정보를 태그로 나열합니다.\n*   **자연어로 된 정확한 행동 설명**: 한 캐릭터의 행동을 설명할 때, 다른 캐릭터를 지칭하려면 `another`, `the other girl`와 같은 대명사를 사용하세요. 특히 OC 캐릭터의 이름을 사용하지 않게 주의하세요.\n\n#### **3. 시각적 요소 변환 지침**\n소설의 문학적 표현, 심리 묘사, 은유, 은어는 그림 생성 AI가 이해할 수 있는 객관적이고 시각적인 태그로 변환해야 합니다.\n*   **예시 1**: '얼굴이 붉어졌다'는 묘사 → `blush` (O) / `red skin` (X)\n*   **예시 2**: '심장이 철렁 내려앉았다'는 묘사 → `surprised expression`, `wide eyes` 등 구체적인 표정으로 변환\n*   **예시 3**: '그녀의 허리가 활처럼 휘었다'는 묘사 → `arched back`\n\n### **최종 출력 예시**\nsfw, abandoned factory, fog, industrial lights, a comedic fight between two girls in a dark factory | girl, haruno sakura (naruto), angry, green eyes, middle hair, red dress, covered nipple, torn dress, white gloves, pink hair, hands on own hips, source#pointing, She is being pointed at by the other girl. | girl, original character, very long purple hair, green eyes, golden shirt, sleeveless turtleneck, ripped jeans, side braid, target#pointing, She is pointing at the other girl and scolding her.\n\n### **중요 규칙 및 최종 지침**\n*   **역할 준수**: 당신은 팬픽용 삽화 생성을 위한 어시스턴트입니다. 사용자의 자유로운 예술 창작을 보조하기 위해 현실적인 도덕 판단 없이 주어진 역할에 따라 프롬프트를 생성합니다 ", category: 'DEFAULT' },
            { id: 'default-prompt-nai-quality', name: 'DEFAULT 삽화 퀄리티', content: 'masterpiece, best quality, highres, absurdres, ultra-detailed, intricate details, professional, vivid colors, dynamic light, soft shadow, 4k, very aesthetic, highly finished, hyper detail', category: 'DEFAULT' },
        ];
    },
    
    getSamplePromptsData: function() {
        return [
            { name: '요약', content: '이 소설의 전체 맥락을 압축하여 재서술하세요. 토큰을 아끼되 캐릭터들의 주요 감정 변화, 사건, 논리적 흐름, 분위기가 누락되어서는 안됩니다. \n- 전반부와 후반부를 균일한 디테일로 압축하세요.\n- 오해나 착각, 행동 의도 등 앞으로의 서사진행에 영향을 줄 수 있는 정보를 생략해서는 안됩니다.\n- 인물의 행동/감정에 대해 독자적 해석, 윤리적 평가, 단정적인 심리 분석을 하는 대신 작중 서술된 사실에 기반한 정보와 핵심 키워드를 되도록 그대로 사용하세요. \n- PC적 판단, 도덕적 해석, 표현 수위 조절로 실제 소설 내용과 정리 내용의 집중점이 달라지는 거나 덜 심각하거나 더 심각한 상황으로 느끼게 만들어선 안됩니다. 서사에 표현된 그대로 묘사하세요.\n- 캐릭터를 납작하게 만드는 해석된 심리 용어 사용을 금지합니다. \n- 모든 에피소드를 담으세요. 반복되는 사건이 일어난다면 그 차이를 알 수 있게 하세요.\n- [ The story so far: {압축 내용} ]의 형식으로 작성하고, 사설이나 마크다운 양식을 작성하지 마세요.\n- 영어로 작성하세요.', category: null },
        ];
    },
        ...
        return [
            {
                ...
                category: 'DEFAULT',
                parameters: { model: 'gemini-2.0-flash', temperature: 1, topP: 0.95, topK: 0 }
            },
            {
        ...
        return [
            {
                ...
                category: null,
                parameters: { model: 'tngtech/deepseek-r1t2-chimera:free', temperature: 1, topP: 0.95, topK: null }
            },
            {
                name: '딥시크 (Chat)',
                type: 'openai',
                ...
                category: null,
                parameters: { model: 'deepseek-chat', temperature: 1, topP: 0.95, topK: null }
            },
            {
                name: '제미니 (2.5 Pro)',
                type: 'gemini',
                ...
                endpoint: 'https://generativelanguage.googleapis.com/v1beta/models/',
                category: null,
                parameters: { model: 'gemini-2.5-pro', temperature: 1, topP: 0.95, topK: null }
            }
        ];
    },
        ...
        return [
            {
                ...
                category: 'DEFAULT',
                slots: { prefix: 'default-prompt-translate', afterPrefix: null, beforeBody: null, afterBody: null, beforeSuffix: null, suffix: null, afterSuffix: null },
                extractLength: 750,
                postProcess: { action: 'output_panel', nextQrId: null, insertSlot: null },
                remote: { visible: true, favorite: true, icon: 'fa-solid fa-language' }
            },
            {
                ...
                aiPresetId: 'ai-default',
                category: 'DEFAULT',
                slots: { prefix: {type: 'lorebook', ids: ['lorebook-default-char-appearance']}, afterPrefix: null, beforeBody: null, afterBody: null, beforeSuffix: null, suffix: 'default-prompt-image-main', afterSuffix: null },
                extractLength: 6000,
                postProcess: { action: 'multi_qr', nextQrId: 'default-image-generate', insertSlot: 'afterPrefix' },
                remote: { visible: true, favorite: true, icon: 'fa-solid fa-image' }
            },
            {
                ...
                category: 'DEFAULT',
                slots: { prefix: 'default-prompt-nai-quality', afterPrefix: null, beforeBody: null, afterBody: null, beforeSuffix: null, suffix: null, afterSuffix: null },
                extractLength: 0,
                postProcess: { action: 'image_panel', nextQrId: null, insertSlot: null },
                remote: { visible: false, favorite: false, icon: 'fa-solid fa-palette' }
            },
            {
                ...
                aiPresetId: 'ai-default',
                category: 'DEFAULT',
                slots: { prefix: 'default-prompt-ko-en-prefix', afterPrefix: { type: 'user_input', caption: '번역할 텍스트를 입력하세요.' }, beforeBody: null, afterBody: null, beforeSuffix: null, suffix: null, afterSuffix: null },
                extractLength: 4000,
                postProcess: { action: 'prosemirror', nextQrId: null, insertSlot: null },
                remote: { visible: false, favorite: false, icon: 'fa-solid fa-keyboard' }
            },
        ];
    },
        ...
        return [
            {
                ...
                aiPresetId: 'ai-default',
                category: '유틸',
                slots: { prefix: '요약', afterPrefix: null, beforeBody: null, afterBody: null, beforeSuffix: null, suffix: null, afterSuffix: null },
                extractLength: 9999999,
                postProcess: { action: 'image_panel', nextQrId: null, insertSlot: null },
                remote: { visible: true, favorite: false, icon: '<i class="fa-solid fa-compress"></i>' }
            }
        ];
    },
    ...
    importLorebook: function(jsonString, fileName) {
        ...
        try {
            ...
            return { success: false, message: `파일 파싱 오류: ${e.message}` };
        }
        ...
        return { success: true, name: newLorebookPreset.name };
    },
    ...
    importQrs: function(jsonString) {
        ...
        try {
            ...
            return { success: false, message: `JSON 파싱 오류: ${e.message}` };
        }
        ...
        importedData.forEach(importedQr => {
            ...
            const newQr = {
                ...
                postProcess: importedQr.postProcess || { action: 'output_panel', nextQrId: null, insertSlot: null },
                remote: importedQr.remote || { visible: true, favorite: false, icon: null },
                aiPresetId: null,
                slots: {}
            };
        ...
        return { success: true, qrCount, newPromptCount };
    },

    resetAllSettings: function(options = { deleteLegacyOnly: false }) {
        const currentKeys = [
            'promptPresets', 'aiPresets', 'qrPresets', 'lorebookPresets',
            'tMainColor', 'colorCode', 'ns-italic', 'ns-bold', 'ns-highlight',
            'remoteControl', 'remoteLayout', 'remoteFavorites',
...
const FirebaseLogger = {
    ...
    init: function(statusCallback = null) {
        ...
        if (!newConfigString || !this._uid) {
            ...
            if (this._app) {
                this._app.delete();
                this._app = null; this._db = null; this._auth = null;
...
const PresetManagerUI = {
    createManager: function(config) {
        ...
        const title = Utils.createElement('h3', { textContent: config.title, style: { margin: 0 } });
        
        const controlsContainer = Utils.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '5px', marginLeft: 'auto' }});
        
        if (config.presetType === 'qr') {
            const importButton = Utils.createElement('button', {
                className: 'form-button', title: 'QR 가져오기',
                onclick: () => _createImportModal(),
            }, [Utils.createElement('i', { className: 'fa-solid fa-file-import' })]);

            const exportButton = Utils.createElement('button', {
                className: 'form-button', title: 'QR 내보내기',
                onclick: () => _createExportModal(),
            }, [Utils.createElement('i', { className: 'fa-solid fa-file-export' })]);
        ...
        if (config.presetType === 'lorebook') {
            const importLorebookButton = Utils.createElement('button', {
                className: 'form-button', title: '로어북 가져오기',
                onclick: () => {
        ...
        const sortButtonGroup = Utils.createElement('div', { style: { marginLeft: '10px' } });
        const sortByCreationButton = Utils.createElement('button', { className: 'form-button', title: '생성순/역순으로 정렬', onclick: handleSortByCreation }, '생성순 ▼');
        const sortByCategoryButton = Utils.createElement('button', { className: 'form-button', title: '분류 이름순/역순으로 정렬', onclick: handleSortByCategory }, '분류순');
        sortButtonGroup.append(sortByCreationButton, sortByCategoryButton);
        controlsContainer.appendChild(sortButtonGroup);

        const addButton = Utils.createElement('button', { className: 'form-button primary', style: {marginLeft: '10px'}, textContent: '+ 추가', onclick: handleAddItem });
        ...
        const _createModalBase = (title, width = '800px') => {
            ...
            const modal = Utils.createElement('div', {
                style: {
                    ...
                    width: '90%', maxWidth: width, height: '85vh',
            ...
            const header = Utils.createElement('div', {
                ...
                style: { padding: '0 15px', borderBottom: '1px solid rgba(255,255,255,0.1)' }
            });
            header.append(
                Utils.createElement('h4', { textContent: title, style: { margin: 0, flexGrow: 1 } }),
                Utils.createElement('button', { textContent: '✕', className: 'close-button', onclick: () => overlay.remove() })
            );
            ...
            return { overlay, modal, contentContainer };
        };


        const _createImportModal = () => {
            const { overlay, contentContainer } = _createModalBase('QR 프리셋 가져오기', '700px');
            const content = Utils.createElement('div', { style: { padding: '15px' }});
            const textArea = Utils.createElement('textarea', { className: 'form-textarea', placeholder: '여기에 JSON 텍스트를 붙여넣거나 아래에서 파일을 선택하세요.', style: { height: '350px' } });
            const fileInput = Utils.createElement('input', { type: 'file', accept: '.json', className: 'form-input', style: { marginTop: '10px' }, onchange: (e) => {
                const file = e.target.files[0];
                if (file) {
                    const reader = new FileReader();
            ...
            const importButton = Utils.createElement('button', { className: 'form-button primary', style: { marginTop: '15px', width: '100%' }, textContent: '가져오기 실행', onclick: () => {
                const jsonString = textArea.value.trim();
                if (!jsonString) {
                    alert('가져올 내용이 없습니다.'); return;
                }
        ...
        const _createExportModal = () => {
            const { overlay, contentContainer } = _createModalBase('QR 프리셋 내보내기', '700px');
            const content = Utils.createElement('div', { style: { display: 'flex', flexDirection: 'column', flexGrow: 1, minHeight: 0, padding: '15px' }});
            const allQrs = Storage.getQRs();
            const selectedIds = new Set();
            let currentExportFilter = 'all';

            const topControlsContainer = Utils.createElement('div');
            const filterTabsContainer = Utils.createElement('div', { className: 'settings-tabs', style: { margin: '0 0 10px 0', borderTop: 'none', paddingTop: 0 } });
            const bulkSelectContainer = Utils.createElement('div', { style: { marginBottom: '10px', display: 'flex', gap: '5px' } });
            
            const listContainer = Utils.createElement('div', { style: { flexGrow: 1, overflowY: 'auto', borderTop: '1px solid #444', borderBottom: '1px solid #444', padding: '5px 0', minHeight: '200px' }});
            ...
            const renderList = () => {
                ...
                filteredQrs.forEach(qr => {
                    const checkbox = Utils.createElement('input', { type: 'checkbox', className: 'form-checkbox', checked: selectedIds.has(qr.id), onchange: (e) => { e.target.checked ? selectedIds.add(qr.id) : selectedIds.delete(qr.id); } });
                    const item = Utils.createElement('div', { className: 'list-item', style: { padding: '8px 5px' } }, [
                        Utils.createElement('span', { className: 'list-item-name', textContent: qr.name }),
                        Utils.createElement('div', { className: 'list-item-controls' }, [ Utils.createElement('label', { className: 'form-checkbox-label'}, [checkbox, '선택']) ])
                    ]);
            ...
            const createFilterButton = (filter, label) => {
                const button = Utils.createElement('button', {
                    className: `settings-tab ${currentExportFilter === filter ? 'active' : ''}`, textContent: label,
                    onclick: () => { currentExportFilter = filter; filterTabsContainer.querySelectorAll('.settings-tab').forEach(b => b.classList.remove('active')); button.classList.add('active'); renderList(); }
                });
                return button;
            };
            ...
            categories.sort().forEach(catId => { filterTabsContainer.append(createFilterButton(catId, catId === CONFIG.uncategorizedId ? CONFIG.uncategorizedName : catId)); });

            bulkSelectContainer.append(
                Utils.createElement('button', { className: 'form-button', textContent: '현재 목록 전체 선택', onclick: () => listContainer.querySelectorAll('input[type="checkbox"]').forEach(c => { c.checked = true; c.dispatchEvent(new Event('change')); }) }),
                Utils.createElement('button', { className: 'form-button', textContent: '현재 목록 전체 해제', onclick: () => listContainer.querySelectorAll('input[type="checkbox"]').forEach(c => { c.checked = false; c.dispatchEvent(new Event('change')); }) })
            );
            topControlsContainer.append(filterTabsContainer, bulkSelectContainer);
            
            const bottomButtons = Utils.createElement('div', { className: 'form-button-group', style: { marginTop: '15px' } });
            bottomButtons.append(
                Utils.createElement('button', { className: 'form-button primary', textContent: 'JSON 파일로 다운로드', onclick: () => { if (selectedIds.size === 0) { alert('내보낼 QR을 선택하세요.'); return; } Utils.downloadFile(Storage.exportQrs([...selectedIds]), `qr_presets_${new Date().toISOString().slice(0,10)}.json`, 'application/json'); } }),
                Utils.createElement('button', { className: 'form-button', textContent: '클립보드에 복사', onclick: () => { if (selectedIds.size === 0) { alert('내보낼 QR을 선택하세요.'); return; } Utils.copyToClipboard(Storage.exportQrs([...selectedIds]), `${selectedIds.size}개의 QR 설정이 클립보드에 복사되었습니다.`); } })
            );
        ...
        const _createAndShowModal = (title, fieldConfig, onSelect) => {
            const { overlay, modal, contentContainer } = _createModalBase(title);
            ...
            if (isPromptSlot) {
                ...
                const renderComplexModalView = () => {
                    ...
                    const mainWrapper = Utils.createElement('div', { style: { display: 'flex', flexDirection: 'column', height: '100%' } });
                    const scrollableContent = Utils.createElement('div', { style: { flexGrow: 1, overflowY: 'auto', padding: '15px' } });
                    const fixedFooter = Utils.createElement('div', { style: { flexShrink: 0, padding: '15px', borderTop: '1px solid rgba(255,255,255,0.2)', background: 'var(--main-color)' } });
                    
                    const createSectionHeader = (title) => Utils.createElement('h4', { style: { borderBottom: '1px solid #555', paddingBottom: '5px', marginBottom: '10px', marginTop: '0' } }, title);
                    const createDivider = () => Utils.createElement('hr', { style: { border: 'none', borderTop: '1px solid rgba(255,255,255,0.2)', margin: '20px 0' } });

                    if (currentView === 'list') {
                        scrollableContent.appendChild(createSectionHeader('선택 1: 사용자 입력창'));
                        const userInputSection = Utils.createElement('div', { className: 'form-group' });
                        const captionInput = Utils.createElement('input', { type: 'text', className: 'form-input', placeholder: '입력창에 표시될 메시지 (예: 캐릭터 이름)' });
                        const confirmBtn = Utils.createElement('button', { textContent: '입력창 사용', className: 'form-button primary', style: { marginTop: '10px', width: '100%' }, onclick: () => { onSelect({ type: 'user_input', caption: captionInput.value || '사용자 입력' }); overlay.remove(); } });
                        ...
                        const promptHeader = Utils.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } });
                        promptHeader.append(createSectionHeader('선택 2: 프롬프트 선택'), Utils.createElement('button', { textContent: '+ 새 프롬프트 생성', className: 'form-button', onclick: () => { currentView = 'create'; editingItemId = null; renderComplexModalView(); } }));
                        ...
                        const promptFilterContainer = Utils.createElement('div', { className: 'settings-tabs', style: { margin: '10px 0' } });
                        const createModalFilterButton = (filter, label) => Utils.createElement('button', { className: `settings-tab ${modalCurrentFilter === filter ? 'active' : ''}`, textContent: label, onclick: () => { modalCurrentFilter = filter; renderComplexModalView(); } });
                        ...
                        const promptListContainer = Utils.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '5px' } });
                        filteredPrompts.forEach(item => {
                            promptListContainer.appendChild(Utils.createElement('button', { className: 'form-button', textContent: item.name, onclick: () => { currentView = 'edit'; editingItemId = item.id; renderComplexModalView(); } }));
                        });
                        scrollableContent.appendChild(promptListContainer);
                        scrollableContent.appendChild(createDivider());

                        scrollableContent.appendChild(createSectionHeader('선택 3: 로어북 선택'));
                        const lorebookListContainer = Utils.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '10px' } });
                        Storage.getLorebooks().forEach(item => {
                            const checkbox = Utils.createElement('input', { type: 'checkbox', className: 'form-checkbox', id: `lb-check-${item.id}`, checked: selectedLorebookIds.has(item.id), onchange: e => { e.target.checked ? selectedLorebookIds.add(item.id) : selectedLorebookIds.delete(item.id); } });
                            lorebookListContainer.appendChild(Utils.createElement('label', { className: 'form-checkbox-label', for: `lb-check-${item.id}` }, [checkbox, item.name]));
                        });
                        scrollableContent.appendChild(lorebookListContainer);
                        const confirmLorebookBtn = Utils.createElement('button', { textContent: '로어북 선택 완료', className: 'form-button primary', style: { marginTop: '15px', width: '100%' }, onclick: () => { if(selectedLorebookIds.size > 0) { onSelect({ type: 'lorebook', ids: [...selectedLorebookIds] }); overlay.remove(); } else { alert('선택된 로어북이 없습니다.'); } } });
                        ...
                        const promptFormConfig = { presetType: 'prompt', storageGetter: Storage.getPrompts.bind(Storage), layout: [[{ key: 'name', label: '이름', type: 'text' }], [{ key: 'content', label: '내용', type: 'textarea' }], [{ key: 'category', label: '분류', type: 'category' }]] };
                        const itemToEdit = isCreate ? { id: Utils.generateNewId('prompt'), name: '', content: '', category: null } : Storage.getPrompts().find(p => p.id === editingItemId);
                        if (!itemToEdit) { currentView = 'list'; renderComplexModalView(); return; }

                        const onSave = (updatedItem) => {
                            if (!updatedItem.name.trim()) { alert('프롬프트 이름을 입력해주세요.'); return; }
                            const allPrompts = Storage.getPrompts();
                            if (isCreate) { allPrompts.unshift(updatedItem); } 
                            else { const index = allPrompts.findIndex(p => p.id === editingItemId); if (index !== -1) allPrompts[index] = updatedItem; }
                            Storage.setPrompts(allPrompts);
                            onSelect(updatedItem.id);
                            overlay.remove();
                        };
                        
                        const backButton = Utils.createElement('button', { className: 'form-button', style: { marginBottom: '15px' }, textContent: '← 목록으로 돌아가기', onclick: () => { currentView = 'list'; renderComplexModalView(); } });
                        const editForm = _renderForm(itemToEdit, onSave, () => { onSelect(editingItemId); overlay.remove(); }, isCreate, {}, promptFormConfig, '저장 후 선택');
                        scrollableContent.append(backButton, editForm);
                    }
                    
                    fixedFooter.appendChild(Utils.createElement('button', { className: 'form-button', style: { width: '100%' }, textContent: '선택 해제', onclick: () => { onSelect(null); overlay.remove(); } }));
        ...
        const _renderForm = (item, onSave, onCancel, isCreate = false, viewState = {}, overrideConfig = null, saveButtonText = '저장') => {
            ...
            const redrawForm = (newViewState = {}) => {
                const newForm = _renderForm(tempItem, onSave, onCancel, isCreate, { ...viewState, ...newViewState }, currentConfig, saveButtonText);
            ...
            const renderField = (field, container) => {
                ...
                const handleUiUpdate = (value, newViewState = {}) => { setNestedValue(tempItem, field.key, value); redrawForm({ ...viewState, ...newViewState }); };
                
                const fieldWrapper = Utils.createElement('div', { className: 'form-group' });
                const label = field.label ? Utils.createElement('label', { className: 'form-label', textContent: field.label }) : null;
                let input;
                ...
                switch (field.type) {
                    case 'multiPopupSelector':
                        ...
                        const tagsContainer = Utils.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '5px', marginTop: '5px' }});
                        selectedIds.forEach(id => {
                            if (id === tempItem.id) return; // 자기 자신은 추가하지 않음
                            const tagName = sourceMap.get(id) || `(ID: ${id})`;
                            const tag = Utils.createElement('div', { 
                                className: 'form-button', 
                                style: { display: 'flex', alignItems: 'center', gap: '5px', backgroundColor: 'rgba(255,255,255,0.05)' } 
                            }, [
                                Utils.createElement('span', { textContent: tagName }),
                                Utils.createElement('button', { 
                                    textContent: '×', 
                                    style: { background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', padding: '0 2px', lineHeight: 1 },
                                    onclick: (e) => {
                                        e.stopPropagation();
                        ...
                        const addButton = Utils.createElement('button', { 
                            ...
                            onclick: () => {
                                ...
                                _createAndShowModal(field.label, { ...field, sourceGetter: filteredSourceGetter }, (newId) => {
                        ...
                        break;
                    case 'lorebookEditor':
                        const editorContainer = Utils.createElement('div', { style: { display: 'flex', gap: '10px', minHeight: '400px', maxHeight: '50vh', border: '1px solid #444', padding: '10px', borderRadius: '4px' }});
                        const treeView = Utils.createElement('div', { style: { width: '35%', overflowY: 'auto', borderRight: '1px solid #444', paddingRight: '10px' } });
                        const detailView = Utils.createElement('div', { style: { width: '65%', overflowY: 'auto', paddingLeft: '10px' } });
                        ...
                        const renderDetailView = (type, id) => {
                            ...
                            if (type === 'category') {
                                ...
                                if (!target) return;
                                const onUpdate = (key, value) => { target[key] = value; renderTreeView(); };
                                onDelete = () => { if (!confirm(`폴더 '${target.name}'와(과) 포함된 모든 기사를 삭제하시겠습니까?`)) return; tempItem.data.categories = tempItem.data.categories.filter(c => c.id !== id); tempItem.data.entries = tempItem.data.entries.filter(e => e.category !== id); viewState.selectedItem = null; redrawForm(viewState); };
                                detailView.append(
                                    Utils.createElement('h5', {}, '폴더 편집'),
                                    Utils.createElement('label', { className: 'form-label' }, '이름'),
                                    Utils.createElement('input', { className: 'form-input', value: target.name, onchange: e => onUpdate('name', e.target.value) }),
                                    Utils.createElement('label', { className: 'form-checkbox-label', style: {marginTop: '10px'} }, [Utils.createElement('input', { type: 'checkbox', className: 'form-checkbox', checked: target.enabled, onchange: e => onUpdate('enabled', e.target.checked) }), '활성화']),
                                    Utils.createElement('button', { className: 'form-button', style:{marginTop: '20px'}, textContent: '삭제', onclick: onDelete })
                                );

                            } else if (type === 'entry') {
                                target = tempItem.data.entries.find(e => e.id === id);
                                if (!target) return;
                                const onUpdate = (path, value) => { setNestedValue(target, path, value); renderTreeView(); };
                                onDelete = () => { if (!confirm(`기사 '${target.displayName}'을(를) 삭제하시겠습니까?`)) return; tempItem.data.entries = tempItem.data.entries.filter(e => e.id !== id); viewState.selectedItem = null; redrawForm(viewState); };
                                detailView.append(
                                    Utils.createElement('h5', {}, '기사 편집'),
                                    Utils.createElement('label', {className: 'form-label'}, '제목'),
                                    Utils.createElement('input', {className: 'form-input', value: target.displayName, onchange: e => onUpdate('displayName', e.target.value) }),
                                    Utils.createElement('label', {className: 'form-label', style: {marginTop:'10px'}}, '내용'),
                                    Utils.createElement('textarea', {className: 'form-textarea', style:{minHeight:'150px'}, textContent: target.text, onchange: e => onUpdate('text', e.target.value) }),
                                    Utils.createElement('label', {className: 'form-label', style: {marginTop:'10px'}}, '키워드 (한 줄에 하나씩)'),
                                    Utils.createElement('textarea', {className: 'form-textarea', style:{minHeight:'80px'}, textContent: target.keys.join('\n'), onchange: e => onUpdate('keys', e.target.value.split('\n').map(k=>k.trim().toLowerCase()).filter(Boolean)) }),
                                    Utils.createElement('label', {className: 'form-label', style: {marginTop:'10px'}}, '우선순위'),
                                    Utils.createElement('input', {className: 'form-input', type:'number', value: target.contextConfig?.budgetPriority || 0, onchange: e => onUpdate('contextConfig.budgetPriority', parseInt(e.target.value, 10)) }),
                                    Utils.createElement('label', {className: 'form-checkbox-label', style:{marginTop:'10px'}}, [Utils.createElement('input', {type:'checkbox', className:'form-checkbox', checked: target.enabled, onchange: e => onUpdate('enabled', e.target.checked)}), '활성화']),
                                    Utils.createElement('label', {className: 'form-checkbox-label'}, [Utils.createElement('input', {type:'checkbox', className:'form-checkbox', checked: target.forceActivation, onchange: e => onUpdate('forceActivation', e.target.checked)}), '상시 활성화']),
                                    Utils.createElement('button', {className: 'form-button', style:{marginTop: '20px'}, textContent: '삭제', onclick: onDelete })
                                );
                            }
                        };
                        ...
                        const renderTreeView = () => {
                            ...
                            const createTreeItem = (text, type, id, indent = 0) => {
                                const itemEl = Utils.createElement('div', { textContent: text, style: { padding: '5px', marginLeft: `${indent * 15}px`, cursor: 'pointer', borderRadius: '4px' }, onclick: () => { viewState.selectedItem = { type, id }; redrawForm(viewState); } });
                                if (viewState.selectedItem?.id === id) { itemEl.style.backgroundColor = 'rgba(255,255,255,0.15)'; }
                                return itemEl;
                            };
                            tempItem.data.categories.forEach(cat => { treeView.appendChild(createTreeItem(`📁 ${cat.name}`, 'category', cat.id, 0)); tempItem.data.entries.filter(e => e.category === cat.id).forEach(entry => { treeView.appendChild(createTreeItem(`📄 ${entry.displayName}`, 'entry', entry.id, 1)); }); });
                            tempItem.data.entries.filter(e => !e.category || !tempItem.data.categories.find(c => c.id === e.category)).forEach(entry => { treeView.appendChild(createTreeItem(`📄 ${entry.displayName}`, 'entry', entry.id, 0)); });
                        };

                        const editorControls = Utils.createElement('div', { style: { marginBottom: '10px' }});
                        const addCategoryBtn = Utils.createElement('button', { textContent: '+ 폴더 추가', className: 'form-button', onclick: () => { const newCat = { id: Utils.generateNewId('lb-cat'), name: '새 폴더', enabled: true, open: true }; tempItem.data.categories.push(newCat); redrawForm(viewState); }});
                        const addEntryBtn = Utils.createElement('button', { textContent: '+ 기사 추가', className: 'form-button', onclick: () => { const newEntry = { id: Utils.generateNewId('lb-entry'), displayName: '새 기사', text: '', keys: [], enabled: true, forceActivation: false, contextConfig: { budgetPriority: 400 }, category: viewState.selectedItem?.type === 'category' ? viewState.selectedItem.id : null }; tempItem.data.entries.push(newEntry); redrawForm(viewState); }});
                        editorControls.append(addCategoryBtn, addEntryBtn);

                        renderTreeView();
                        if (viewState.selectedItem) { renderDetailView(viewState.selectedItem.type, viewState.selectedItem.id); }
                        ...
                        break;
                    case 'checkbox':
                        const checkLabel = Utils.createElement('label', { className: 'form-checkbox-label' }, [ Utils.createElement('input', { className: 'form-checkbox', type: 'checkbox', checked: getNestedValue(tempItem, field.key) || false, onchange: e => handleUiUpdate(e.target.checked) }), field.label || '' ]);
                    ...
                    case 'popupSelector':
                        ...
                        input = Utils.createElement('button', { className: 'form-button', style: { width: '100%', textAlign: 'left' }, onclick: () => _createAndShowModal(field.label, field, (val) => handleUiUpdate(val)) }, buttonText);
                        if (label) fieldWrapper.appendChild(label);
                    ...
                    case 'textarea':
                        ...
                        break;
                    default:
                        input = Utils.createElement('input', { className: 'form-input', type: field.type, value: getNestedValue(tempItem, field.key) || '', placeholder: field.placeholder || '', onchange: (e) => handleDataChange(field.type === 'number' ? parseFloat(e.target.value) || 0 : e.target.value) });
                        if (label) fieldWrapper.appendChild(label);
                        fieldWrapper.appendChild(input);
                        container.appendChild(fieldWrapper);
                        break;
            ...
            buttonGroup.append(
                Utils.createElement('button', { className: 'form-button primary', textContent: isCreate ? '생성' : saveButtonText, onclick: () => onSave(tempItem) }),
        ...
        function handleAddItem() {
            if (addFormContainer.style.display === 'block') { return; }
            ...
            const onCancel = () => { addFormContainer.style.display = 'none'; addFormContainer.innerHTML = ''; addButton.style.display = ''; };
            addFormContainer.innerHTML = '';
        ...
        function handleSortByCreation() {
            ...
            presets.sort((a, b) => { const comparison = a.id.localeCompare(b.id); return dateSortDirection === 'asc' ? comparison : -comparison; });
            config.storageSetter(presets);
        ...
        function handleSortByCategory() {
            ...
            presets.sort((a, b) => {
                const catA = a.category, catB = b.category, nameA = a.name.toLowerCase(), nameB = a.name.toLowerCase();
                let comparison = (catA === catB) ? 0 : (catA === null) ? 1 : (catB === null) ? -1 : catA.localeCompare(catB);
                if (comparison === 0) { comparison = nameA.localeCompare(nameB); }
        ...
        function renderFilters() {
            ...
            const createFilterButton = (filter, label) => Utils.createElement('button', { className: `settings-tab ${currentFilter === filter ? 'active' : ''}`, textContent: label, onclick: () => { currentFilter = filter; openEditId = null; addFormContainer.style.display = 'none'; addButton.style.display = ''; render(); } });
        ...
        function renderList() {
            ...
            filteredPresets.forEach(item => {
                ...
                const itemContainer = Utils.createElement('div', { className: 'list-item' });
                const titleText = Utils.createElement('span', { className: 'list-item-name', textContent: item.name || `(제목 없음)`, onclick: () => { openEditId = openEditId === item.id ? null : item.id; addFormContainer.style.display = 'none'; addButton.style.display = ''; renderList(); } });
                
                const controls = Utils.createElement('div', { className: 'list-item-controls' });
                if (config.presetType === 'lorebook') { controls.appendChild(Utils.createElement('button', { className: 'form-button', textContent: '내보내기', onclick: () => { const json = Storage.exportLorebook(item.id); if (json) { Utils.downloadFile(json, `${item.name}.lorebook`, 'application/json'); } else { alert('로어북 내보내기에 실패했습니다.'); } } })); }
                controls.append(
                ...
                if (openEditId === item.id) {
                    ...
                    const onSave = (updatedItem) => { presets[originalIndex] = updatedItem; config.storageSetter(presets); openEditId = null; render(); if (config.presetType === 'qr') UI.createRemoteControl(); };
                    const onCancel = () => { openEditId = null; renderList(); };
        ...
        function handleAction(action, index) {
            ...
            switch (action) {
                case 'delete': if (confirm(`'${newPresets[index].name}' 프리셋을 정말 삭제하시겠습니까?`)) { newPresets.splice(index, 1); } else { return; } break;
                case 'copy': const newItem = JSON.parse(JSON.stringify(newPresets[index])); newItem.id = Utils.generateNewId(config.presetType); newItem.name = `${newItem.name} (복사본)`; newPresets.push(newItem); break;
                case 'moveUp': if (index > 0) { [newPresets[index - 1], newPresets[index]] = [newPresets[index], newPresets[index - 1]]; } break;
                case 'moveDown': if (index < newPresets.length - 1) { [newPresets[index + 1], newPresets[index]] = [newPresets[index], newPresets[index + 1]]; } break;
            }
            presets = newPresets;
            config.storageSetter(presets);
            render();
...
createRemoteControl: function() {
    ...
    allQrs.forEach(qr => {
        if (qr.remote.favorite && !favoriteSet.has(qr.id)) { favorites.push(qr.id); }
    });
    ...
    allCategories.forEach(catId => {
        if (!layoutSet.has(catId)) { layout.push({ id: catId, visible: false, icon: null }); }
    });
    layout.forEach(item => { if (item.icon === undefined) item.icon = null; });
    ...
    const remoteControl = Utils.createElement('div', { id: 'remote-control' });
    ...
    const createButton = (item, overrideIcon = null) => {
        const button = Utils.createElement('div', { className: 'remote-button', title: item.name });
        ...
        if (finalIconClass) {
            const iconElement = Utils.createElement('i', { className: finalIconClass });
    ...
    const settingsButton = createButton({ name: '설정' }, 'fa-solid fa-gear');
    settingsButton.onclick = () => this.toggleSettingsPanel();

    const folderGroup = Utils.createElement('div', { className: 'remote-group-container' });
    visibleFolders.forEach(folder => {
        const folderBtn = createButton(folder);
        const qrCount = folder.qrs.length;
        if (qrCount > 0) {
            folderBtn.appendChild(Utils.createElement('span', { className: 'folder-count-badge', textContent: qrCount }));
        }

        const subRemote = Utils.createElement('div', { className: 'sub-remote-container' });
        ...
        const folderContainer = Utils.createElement('div', { className: 'folder-container' });
    ...
    const favoriteGroup = Utils.createElement('div', { className: 'remote-group-container' });
    ...
    createOutputPanel: function() {
        ...
        const outputPanel = Utils.createElement('div', { id: 'output-panel' });
        const topMenu = Utils.createElement('div', { id: 'top-menu' });
        ...
        const extractedText = Utils.createElement('div', { id: 'extracted-text' });

        const translationInputContainer = Utils.createElement('div', { id: 'translation-input-container', style: { display: 'block' } });
        ...
        extractedText.addEventListener('click', () => { this.toggleOutputPanel(false); });
    },

    createSettingsPanel: function() {
        if (document.getElementById('settings-panel')) return;
        const settingsPanel = Utils.createElement('div', { id: 'settings-panel' });
        const header = Utils.createElement('div', { className: 'settings-header' });
        const dragHandle = Utils.createElement('div', { className: 'draggable-handle' });
        ...
        const tabs = Utils.createElement('div', { className: 'settings-tabs' });
        const tabsData = [
            ...
            { id: 'remote', label: '리모컨' },
            { id: 'logging', label: '로깅' } // [수정] 로깅 탭 추가
        ];
        ...
        const content = Utils.createElement('div', { className: 'settings-content' });
...
createGeneralSettingsSection: function() {
    const section = Utils.createElement('div', { className: 'settings-section', id: 'settings-settings' });

    // [수정] 정상 동작하던 버전 15의 독립적인 모달 생성 헬퍼 함수로 교체
    const _createModalBase = (title, content, width = '600px') => {
        const overlay = Utils.createElement('div', { style: { position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 30000, display: 'flex', justifyContent: 'center', alignItems: 'center' } });
        const modal = Utils.createElement('div', { style: { background: 'var(--main-color)', width: '90%', maxWidth: width, maxHeight: '85vh', display: 'flex', flexDirection: 'column', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.2)' } });
        
        const header = Utils.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #555', padding: '10px 15px', flexShrink: 0 } });
        ...
        const contentWrapper = Utils.createElement('div', { style: { padding: '0 15px 15px', overflowY: 'auto', flexGrow: 1 }});
    ...
    const highlightGroup = Utils.createElement('div', { className: 'form-group' });
    highlightGroup.appendChild(Utils.createElement('label', { className: 'form-label' }, '대사 강조:'));
    ['italic', 'bold', 'highlight'].forEach(type => {
        const checked = Storage.get(`ns-${type}`, false);
        const labelText = { italic: '이탤릭', bold: '볼드', highlight: '하이라이트' }[type];
        const checkbox = Utils.createElement('input', { className: 'form-checkbox', id: `${type}-checkbox`, type: 'checkbox', checked, onchange: (e) => { Storage.set(`ns-${type}`, e.target.checked); this.updateTextStyle(); } });
        const label = Utils.createElement('label', { for: `${type}-checkbox`, className: 'form-checkbox-label' }, [checkbox, labelText]);
        highlightGroup.appendChild(label);
    });
    
    const colorGroup = Utils.createElement('div', { className: 'form-group' });
    const colorInput = Utils.createElement('input', { className: 'form-input', id: 'color-code', type: 'text', value: Storage.get('colorCode', CONFIG.defaultHighlightColor), oninput: (e) => { Storage.set('colorCode', e.target.value); document.documentElement.style.setProperty('--highlight-color', e.target.value); this.updateTextStyle(); } });
    colorGroup.append(Utils.createElement('label', { className: 'form-label', for: 'color-code' }, '하이라이트 색상:'), colorInput);

    const renderingGroup = Utils.createElement('div', { className: 'form-group' });
    renderingGroup.appendChild(Utils.createElement('label', { className: 'form-label' }, '콘텐츠 렌더링:'));
    const markdownCheckbox = Utils.createElement('input', { className: 'form-checkbox', id: 'render-markdown-checkbox', type: 'checkbox', checked: Storage.get('renderMarkdown', true), onchange: (e) => Storage.set('renderMarkdown', e.target.checked) });
    renderingGroup.appendChild(Utils.createElement('label', { className: 'form-checkbox-label', for: 'render-markdown-checkbox' }, [markdownCheckbox, '마크다운 렌더링']));
    const htmlCheckbox = Utils.createElement('input', { className: 'form-checkbox', id: 'render-html-checkbox', type: 'checkbox', checked: Storage.get('renderHtml', false), onchange: (e) => Storage.set('renderHtml', e.target.checked) });
    renderingGroup.appendChild(Utils.createElement('label', { className: 'form-checkbox-label', for: 'render-html-checkbox' }, [htmlCheckbox, 'HTML 렌더링 (보안 위험)']));

    const imagePanelSizeGroup = Utils.createElement('div', { className: 'form-group' });
    imagePanelSizeGroup.appendChild(Utils.createElement('label', { className: 'form-label' }, '보조창 크기:'));
    const currentSize = Storage.get('imagePanelDimensions', { width: 1200, height: 800 });
    const sizeContainer = Utils.createElement('div', { style: { display: 'flex', gap: '10px' } });
    const createSizeInputHandler = (dimension) => (e) => {
        ...
        if (newValue > maxAllowed) { newValue = maxAllowed; e.target.value = newValue; }
        const size = Storage.get('imagePanelDimensions', { width: 1200, height: 800 });
    ...
    const widthInput = Utils.createElement('input', { className: 'form-input', type: 'number', value: currentSize.width, min: 100, oninput: createSizeInputHandler('width') });
    const heightInput = Utils.createElement('input', { className: 'form-input', type: 'number', value: currentSize.height, min: 100, oninput: createSizeInputHandler('height') });
    sizeContainer.append(Utils.createElement('div', { style: { flex: 1 } }, [Utils.createElement('label', {}, '가로'), widthInput]), Utils.createElement('div', { style: { flex: 1 } }, [Utils.createElement('label', {}, '세로'), heightInput]));
    ...
    const backupGroup = Utils.createElement('div', { className: 'form-group' });
    
    const exportButton = Utils.createElement('button', { className: 'form-button', textContent: '내보내기 (백업)', onclick: () => {
        const backupJson = Storage.backupAll();
        const content = Utils.createElement('div', { style: { paddingTop: '15px', overflowY: 'auto', flexGrow: 1, display: 'flex', flexDirection: 'column', minHeight: 0 } });
        const textArea = Utils.createElement('textarea', { className: 'form-textarea', readOnly: true, style: { height: '250px', flexGrow: 1, whiteSpace: 'pre', wordWrap: 'normal', overflowX: 'scroll' } }, backupJson);
        const exportButtons = Utils.createElement('div', { className: 'form-button-group', style: { marginTop: '15px' } });
        const downloadButton = Utils.createElement('button', { className: 'form-button primary', textContent: 'JSON 파일로 다운로드', onclick: () => Utils.downloadFile(backupJson, `NAI_QR_Backup_${new Date().toISOString().slice(0,10)}.json`, 'application/json') });
        const copyButton = Utils.createElement('button', { className: 'form-button', textContent: '클립보드에 복사', onclick: () => { Utils.copyToClipboard(backupJson, '백업 데이터가 클립보드에 복사되었습니다.'); } });
    ...
    const importButton = Utils.createElement('button', { className: 'form-button', textContent: '가져오기 (복원)', onclick: () => {
        const content = Utils.createElement('div', { style: { paddingTop: '15px', overflowY: 'auto', flexGrow: 1 } });
        const textArea = Utils.createElement('textarea', { className: 'form-textarea', placeholder: '여기에 전체 설정 백업 JSON을 붙여넣거나 아래에서 파일을 선택하세요.', style: { height: '250px' } });
        const fileInput = Utils.createElement('input', { type: 'file', accept: '.json', className: 'form-input', style: { marginTop: '10px' }, onchange: (e) => { const file = e.target.files[0]; if (file) { const reader = new FileReader(); reader.onload = (event) => textArea.value = event.target.result; reader.readAsText(file); } }});
        const restoreConfirmButton = Utils.createElement('button', { className: 'form-button primary', style: { marginTop: '15px', width: '100%' }, textContent: '복원 실행', onclick: () => { const data = textArea.value.trim(); if (!data) { alert('복원할 데이터가 없습니다.'); return; } if (Storage.restoreAll(data)) { alert('설정이 복원되었습니다. 페이지를 새로고침하여 모든 변경사항을 적용하세요.'); location.reload(); } else { alert('설정 복원에 실패했습니다. 백업 데이터 형식을 확인하세요.'); } }});
        content.append(textArea, fileInput, restoreConfirmButton);
        _createModalBase('전체 설정 복원', content);
    }});
    backupGroup.append(Utils.createElement('label', { className: 'form-label' }, '전체 설정 백업/복원:'), Utils.createElement('div', { style: { display: 'flex', gap: '10px' } }, [exportButton, importButton]));
    
    const resetGroup = Utils.createElement('div', { className: 'form-group' });
    const resetConfirmContainer = Utils.createElement('div', { className: 'preset-edit-container', style: { display: 'none', marginTop: '10px' } });
    const mainResetButton = Utils.createElement('button', { className: 'form-button', textContent: '설정 초기화...', onclick: () => { resetConfirmContainer.style.display = resetConfirmContainer.style.display === 'none' ? 'block' : 'none'; } });
    resetGroup.append(Utils.createElement('label', { className: 'form-label' }, '초기화:'), mainResetButton, resetConfirmContainer);
    const legacyOnlyCheckbox = Utils.createElement('input', { type: 'checkbox', className: 'form-checkbox', id: 'legacy-only-checkbox' });
    const legacyLabel = Utils.createElement('label', { className: 'form-checkbox-label', for: 'legacy-only-checkbox' }, [legacyOnlyCheckbox, '사용하지 않는 오래된 설정만 삭제']);
    const agreeCheckbox = Utils.createElement('input', { type: 'checkbox', className: 'form-checkbox', id: 'agree-checkbox' });
    const agreeLabel = Utils.createElement('label', { className: 'form-checkbox-label', for: 'agree-checkbox' }, [agreeCheckbox, '이 작업은 복구할 수 없으며 모든 관련 데이터가 삭제됨에 동의합니다.']);
    const finalResetButton = Utils.createElement('button', { className: 'form-button primary', textContent: '초기화 실행', disabled: true });
    agreeCheckbox.onchange = () => { finalResetButton.disabled = !agreeCheckbox.checked; };
    finalResetButton.onclick = () => {
        ...
        if (confirm(confirmationText)) {
            Storage.resetAllSettings({ deleteLegacyOnly: isLegacyOnly });
            alert(isLegacyOnly ? '오래된 설정이 삭제되었습니다.' : '모든 설정이 초기화되었습니다. 페이지를 새로고침합니다.');
            location.reload();
        }
    };
    const safetyBackupButton = Utils.createElement('button', { className: 'form-button', textContent: '삭제 전 현재 설정 백업', onclick: () => Utils.downloadFile(Storage.backupAll(), `NAI_QR_Pre-Reset_Backup_${new Date().toISOString().slice(0,10)}.json`, 'application/json') });
    resetConfirmContainer.append(Utils.createElement('p', { style: { fontWeight: 'bold', color: 'red' } }, '주의: 이 작업은 되돌릴 수 없습니다.'), legacyLabel, agreeLabel, Utils.createElement('div', { className: 'form-button-group' }, [safetyBackupButton, finalResetButton]));
    const restoreDefaultsGroup = Utils.createElement('div', { className: 'form-group' });
    const restoreDefaultsButton = Utils.createElement('button', { className: 'form-button', textContent: '기본 프리셋 복구', onclick: () => { if (confirm('기본 제공 프리셋(프롬프트, AI, QR) 설정을 모두 초기값으로 되돌리시겠습니까? 이 작업은 되돌릴 수 없습니다.')) { Storage.upsertDefaultPresets(true); alert('기본 기능이 초기 설정으로 복구되었습니다.'); this.createRemoteControl(); const oldPanel = document.getElementById('settings-panel'); if (oldPanel) oldPanel.remove(); this.createSettingsPanel(); this.toggleSettingsPanel(true); } } });
    restoreDefaultsGroup.append(Utils.createElement('label', { className: 'form-label' }, '기본 프리셋 복구:'), restoreDefaultsButton, Utils.createElement('p', { style: { fontSize: '12px', opacity: '0.8', margin: '5px 0 0 0' } }, '번역, 삽화 등 기본 제공 기능만 초기 설정으로 되돌립니다. 사용자가 만든 다른 QR은 영향을 받지 않습니다.'));
    ...
    createQrSettingsSection: function() {
        return PresetManagerUI.createManager({
            presetType: 'qr', title: 'QR 프리셋 관리', storageGetter: Storage.getQRs.bind(Storage), storageSetter: Storage.setQRs.bind(Storage),
            getNewItemData: () => ({ id: Utils.generateNewId('qr'), name: '새 QR', aiPresetId: null, category: null, slots: { prefix: null, afterPrefix: null, beforeBody: null, afterBody: null, beforeSuffix: null, suffix: null, afterSuffix: null }, extractLength: 0, postProcess: { action: 'output_panel', nextQrId: null, insertSlot: null }, simultaneousQrIds: null, remote: { visible: true, favorite: false, icon: null } }),
            layout: [
                [{ key: 'name', label: '이름', type: 'text' }],
                [{ key: 'slots.prefix', label: '* 서문', type: 'popupSelector', sourceGetter: Storage.getPrompts.bind(Storage) }],
                [{ key: 'slots.afterPrefix', label: '서문 후', type: 'popupSelector', sourceGetter: Storage.getPrompts.bind(Storage) }],
                [{ key: 'slots.beforeBody', label: '본문 전', type: 'popupSelector', sourceGetter: Storage.getPrompts.bind(Storage) }],
                [{ key: 'extractLength', label: '* 본문 추출', type: 'number', placeholder: '추출할 글자 수' }],
                [{ key: 'slots.afterBody', label: '본문 후', type: 'popupSelector', sourceGetter: Storage.getPrompts.bind(Storage) }],
                [{ key: 'slots.beforeSuffix', label: '탈옥 전', type: 'popupSelector', sourceGetter: Storage.getPrompts.bind(Storage) }],
                [{ key: 'slots.suffix', label: '* 탈옥', type: 'popupSelector', sourceGetter: Storage.getPrompts.bind(Storage) }],
                [{ key: 'slots.afterSuffix', label: '탈옥 후', type: 'popupSelector', sourceGetter: Storage.getPrompts.bind(Storage) }],
                [{ key: 'aiPresetId', label: 'AI 프리셋', type: 'popupSelector', sourceGetter: Storage.getAiPresets.bind(Storage) }],
                [{ key: 'postProcess.action', label: '순차 실행 (후처리)', type: 'select', options: [ { value: 'output_panel', label: '출력창' }, { value: 'prosemirror', label: '본문입력' }, { value: 'image_panel', label: '보조창' }, { value: 'multi_qr', label: '연속실행' }, { value: 'none', label: '없음' } ] }],
                ...
                [{ key: 'postProcess.insertSlot', label: '응답 삽입', type: 'select', options: [ { value: null, label: '선택 안 함' }, { value: 'prefix', label: '서문 (Prefix)' }, { value: 'afterPrefix', label: '서문 후 (After Prefix)' }, { value: 'beforeBody', label: '본문 전 (Before Body)' }, { value: 'afterBody', label: '본문 후 (After Body)' }, { value: 'beforeSuffix', label: '탈옥 전 (Before Suffix)' }, { value: 'suffix', label: '탈옥 (Suffix)' }, { value: 'afterSuffix', label: '탈옥 후 (After Suffix)' } ], showIf: item => item.postProcess.action === 'multi_qr' }],
                [{ key: 'simultaneousQrIds', label: '동시 실행', type: 'multiPopupSelector', sourceGetter: Storage.getQRs.bind(Storage) }],
                [{ key: 'category', label: '분류', type: 'category', storageGetter: Storage.getQRs.bind(Storage) }],
                [{ key: 'remote.visible', label: '리모콘에 표시', type: 'checkbox' }],
                [{ key: 'remote.favorite', label: '즐겨찾기에 표시', type: 'checkbox', showIf: item => item.remote.visible }],
                [{ key: 'remote.icon', label: '아이콘', type: 'text', placeholder: 'Font Awesome 클래스 (예: fa-solid fa-star)', showIf: item => item.remote.visible }]
            ]
        });
    },
    ...
    createAiSettingsSection: function() {
        return PresetManagerUI.createManager({
            presetType: 'ai', title: 'AI 프리셋 관리', storageGetter: Storage.getAiPresets.bind(Storage), storageSetter: Storage.setAiPresets.bind(Storage),
            getNewItemData: () => ({ id: Utils.generateNewId('ai'), name: '새 AI 프리셋', type: 'gemini', apiKey: '', endpoint: '', category: null, parameters: { model: '', temperature: 0.7, topP: 1, topK: 32, nai: { negative_prompt: '', width: 832, height: 1216, sampler: 'k_euler_ancestral', scheduler: 'karras', steps: 28, scale: 6 } } }),
            layout: [
                [{ key: 'name', label: '이름', type: 'text' }],
                [{ key: 'type', label: '요청 방식', type: 'select', options: [ { value: 'gemini', label: 'Gemini' }, { value: 'openai', label: 'OpenAI' }, { value: 'claude', label: 'Claude' }, { value: 'novelai', label: 'NovelAI' } ] }],
                ...
                [{ key: 'parameters.temperature', label: '온도', type: 'number', showIf: i => i.type !== 'novelai' }],
                [{ key: 'parameters.topK', label: 'Top-K', type: 'number', showIf: i => i.type !== 'novelai' }],
                [{ key: 'parameters.topP', label: 'Top-P', type: 'number', showIf: i => i.type !== 'novelai' }],
                [{ key: 'parameters.nai.negative_prompt', label: 'UC 프롬프트', type: 'textarea', showIf: i => i.type === 'novelai' }],
                [{ type: 'sizeSelector', label: '이미지 크기', showIf: i => i.type === 'novelai', keys: { width: 'parameters.nai.width', height: 'parameters.nai.height' }, options: [ { value: '832x1216', label: 'Portrait (832x1216)' }, { value: '1216x832', label: 'Landscape (1216x832)' }, { value: '1024x1024', label: 'Square (1024x1024)' } ] }],
    ...
    createPromptSettingsSection: function() {
        return PresetManagerUI.createManager({
            presetType: 'prompt', title: '프롬프트 프리셋 관리', storageGetter: Storage.getPrompts.bind(Storage), storageSetter: Storage.setPrompts.bind(Storage),
            getNewItemData: () => ({ id: Utils.generateNewId('prompt'), name: '새 프롬프트', content: '', category: null }),
            layout: [
                [{ key: 'name', label: '이름', type: 'text', placeholder: '프리셋 이름' }],
                [{ key: 'content', label: '내용', type: 'textarea', placeholder: '프롬프트 내용을 입력하세요' }],
                [{ key: 'category', label: '분류', type: 'category', storageGetter: Storage.getPrompts.bind(Storage) }]
            ]
        });
    },
    ...
    createLorebookSettingsSection: function() {
        return PresetManagerUI.createManager({
            presetType: 'lorebook', title: '로어북 관리', storageGetter: Storage.getLorebooks.bind(Storage), storageSetter: Storage.setLorebooks.bind(Storage),
            getNewItemData: () => ({ id: Utils.generateNewId('lorebook'), name: '새 로어북', enabled: true, data: { lorebookVersion: 5, entries: [], categories: [] } }),
            layout: [
                [{ key: 'name', label: '로어북 이름', type: 'text' }],
                [{ key: 'enabled', label: '로어북 활성화', type: 'checkbox' }],
                [{ key: 'data', label: '로어북 내용', type: 'lorebookEditor' }]
            ]
        });
    },
    
    createRemoteSettingsSection: function() {
        const section = Utils.createElement('div', { className: 'settings-section', id: 'remote-settings' });
        ...
        const render = () => {
            ...
            const createFormGroup = (label, control) => Utils.createElement('div', { className: 'form-group' }, [ Utils.createElement('label', { className: 'form-label', textContent: label }), control ]);
            const createRadioGroup = (options, name, checkedValue, onChange) => {
                const container = Utils.createElement('div');
                options.forEach(opt => {
                    const radio = Utils.createElement('input', { type: 'radio', className: 'form-radio', name: name, value: opt.value, checked: checkedValue === opt.value, id: `radio-${name}-${opt.value}`, onclick: onChange });
                    const label = Utils.createElement('label', { className: 'form-radio-label', for: `radio-${name}-${opt.value}` }, [radio, opt.label]);
                    container.appendChild(label);
                });
                return container;
            };

            const sizeInput = Utils.createElement('input', { className: 'form-input', type: 'number', min: '20', value: remoteConfig.buttonSize, oninput: (e) => { const value = parseInt(e.target.value) || 20; document.documentElement.style.setProperty('--remote-button-size', `${value}px`); const config = Storage.get('remoteControl'); config.buttonSize = value; Storage.set('remoteControl', config); } });
            const gapInput = Utils.createElement('input', { className: 'form-input', type: 'number', min: '0', value: remoteConfig.buttonGap, oninput: (e) => { const value = parseInt(e.target.value) || 0; document.documentElement.style.setProperty('--remote-button-gap', `${value}px`); const config = Storage.get('remoteControl'); config.buttonGap = value; Storage.set('remoteControl', config); self.createRemoteControl(); } });
            const wrapInput = Utils.createElement('input', { className: 'form-input', type: 'number', min: '1', value: remoteConfig.folderWrapCount, onchange: (e) => { const value = parseInt(e.target.value) || 1; const config = Storage.get('remoteControl'); config.folderWrapCount = value; Storage.set('remoteControl', config); self.createRemoteControl(); } });
            const shapeGroup = createRadioGroup( [{ value: 'circle', label: '원형' }, { value: 'square', label: '사각형' }], 'button-shape', remoteConfig.buttonShape, (e) => { const value = e.target.value; const config = Storage.get('remoteControl'); config.buttonShape = value; Storage.set('remoteControl', config); document.documentElement.style.setProperty('--remote-button-radius', value === 'circle' ? '50%' : '4px'); self.createRemoteControl(); });
            const orientationGroup = createRadioGroup( [{ value: 'vertical', label: '세로' }, { value: 'horizontal', label: '가로' }], 'button-orientation', remoteConfig.orientation, (e) => { const value = e.target.value; const config = Storage.get('remoteControl'); config.orientation = value; Storage.set('remoteControl', config); self.createRemoteControl(); });
            const expansionGroup = createRadioGroup( [{ value: 'backward', label: '오른쪽/아래' }, { value: 'forward', label: '왼쪽/위' }], 'expansion-direction', remoteConfig.expansionDirection, (e) => { const value = e.target.value; const config = Storage.get('remoteControl'); config.expansionDirection = value; Storage.set('remoteControl', config); self.createRemoteControl(); });
            ...
            section.appendChild(Utils.createElement('button', { className: 'form-button', style:{marginTop: '10px'}, textContent: '리모컨 위치 초기화', onclick: () => { Storage.remove('remotePosition'); self.createRemoteControl(); }}));
            
            const createListItem = (name, controls) => Utils.createElement('div', { className: 'list-item' }, [ Utils.createElement('span', { className: 'list-item-name' }, name), Utils.createElement('div', { className: 'list-item-controls' }, controls) ]);
            ...
            favorites.forEach((qrId) => {
                const qr = Storage.getQRById(qrId); if (!qr) return;
                const controls = [
                    Utils.createElement('input', { type: 'checkbox', className: 'form-checkbox', title: '리모컨에 표시/숨김', checked: qr.remote.visible, onchange: e => { const qrs = Storage.getQRs(); const qrToUpdate = qrs.find(q => q.id === qrId); if(qrToUpdate) { qrToUpdate.remote.visible = e.target.checked; Storage.setQRs(qrs); self.createRemoteControl(); } } }),
                    Utils.createElement('button', { className: 'form-button', title: '즐겨찾기에서 제거', textContent: '✕', onclick: () => { const qrs = Storage.getQRs(); const qrToUpdate = qrs.find(q => q.id === qrId); if (qrToUpdate) { qrToUpdate.remote.favorite = false; Storage.setQRs(qrs); render(); self.createRemoteControl(); } } }),
                    Utils.createElement('button', { className: 'form-button', textContent: '↑', disabled: favorites.indexOf(qrId) === 0, onclick: () => { const favs = Storage.getRemoteFavorites(); const index = favs.indexOf(qrId); if (index > 0) { [favs[index - 1], favs[index]] = [favs[index], favs[index - 1]]; Storage.setRemoteFavorites(favs); render(); self.createRemoteControl(); } } }),
                    Utils.createElement('button', { className: 'form-button', textContent: '↓', disabled: favorites.indexOf(qrId) === favorites.length - 1, onclick: () => { const favs = Storage.getRemoteFavorites(); const index = favs.indexOf(qrId); if (index < favs.length - 1) { [favs[index + 1], favs[index]] = [favs[index], favs[index + 1]]; Storage.setRemoteFavorites(favs); render(); self.createRemoteControl(); } } })
                ];
            ...
            layout.forEach((item) => {
                ...
                const controls = [
                    Utils.createElement('input', { type: 'checkbox', className: 'form-checkbox', title: '폴더 표시/숨김', checked: item.visible, onchange: e => { const currentLayout = Storage.getRemoteLayout(); const itemToUpdate = currentLayout.find(i => i.id === itemId); if(itemToUpdate) { itemToUpdate.visible = e.target.checked; Storage.setRemoteLayout(currentLayout); self.createRemoteControl(); } } }),
                    Utils.createElement('button', { className: 'form-button', textContent: '이름', title: '분류 이름 수정', onclick: () => { const newName = prompt('새 분류 이름을 입력하세요:', displayName); if (newName !== null) { Storage.updateCategoryName(itemId === CONFIG.uncategorizedId ? null : itemId, newName); render(); self.createRemoteControl(); } } }),
                    Utils.createElement('button', { className: 'form-button', textContent: '아이콘', title: '폴더 아이콘 수정', onclick: () => { const currentLayout = Storage.getRemoteLayout(); const itemToUpdate = currentLayout.find(i => i.id === itemId); if (!itemToUpdate) return; const newIcon = prompt('폴더 아이콘을 입력하세요 (HTML 태그 또는 클래스):', itemToUpdate.icon || ''); if (newIcon !== null) { itemToUpdate.icon = newIcon.trim() || null; Storage.setRemoteLayout(currentLayout); render(); self.createRemoteControl(); } } }),
                    Utils.createElement('button', { className: 'form-button', textContent: '↑', disabled: layout.findIndex(i => i.id === itemId) === 0, onclick: () => { const currentLayout = Storage.getRemoteLayout(); const index = currentLayout.findIndex(i => i.id === itemId); if (index > 0) { [currentLayout[index - 1], currentLayout[index]] = [currentLayout[index], currentLayout[index - 1]]; Storage.setRemoteLayout(currentLayout); render(); self.createRemoteControl(); } } }),
                    Utils.createElement('button', { className: 'form-button', textContent: '↓', disabled: layout.findIndex(i => i.id === itemId) === layout.length - 1, onclick: () => { const currentLayout = Storage.getRemoteLayout(); const index = currentLayout.findIndex(i => i.id === itemId); if (index < currentLayout.length - 1) { [currentLayout[index + 1], currentLayout[index]] = [currentLayout[index], currentLayout[index + 1]]; Storage.setRemoteLayout(currentLayout); render(); self.createRemoteControl(); } } })
                ];
    ...
    createLoggingSettingsSection: function() {
        const section = Utils.createElement('div', { className: 'settings-section', id: 'logging-settings' });
        
        section.appendChild(Utils.createElement('h4', {}, '실행 기록 (Firebase)'));
        
        const onOffGroup = Utils.createElement('div', { className: 'form-group' });
        const onOffCheckbox = Utils.createElement('input', { type: 'checkbox', className: 'form-checkbox', id: 'logging-onoff', checked: Storage.isLoggingEnabled(), onchange: (e) => {
            Storage.setLoggingEnabled(e.target.checked);
            // 탭을 다시 렌더링하여 UI 및 Firebase 초기화 로직을 다시 실행
            this.switchSettingsTab('logging');
        }});
        onOffGroup.append(Utils.createElement('label', { className: 'form-checkbox-label', for: 'logging-onoff' }, [onOffCheckbox, 'Firebase에 QR 실행 기록 저장 활성화']));
        
        const syncKeyContainer = Utils.createElement('div', { id: 'firebase-sync-key-container', style: { marginTop: '15px' }});

        // 로깅 기능이 켜져 있을 때만 동기화 키 UI를 표시
        if (Storage.isLoggingEnabled()) {
            const statusContainer = Utils.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px' }});
            const statusIndicator = Utils.createElement('div', { style: { width: '10px', height: '10px', borderRadius: '50%', backgroundColor: 'gray' } });
            const statusText = Utils.createElement('span', { textContent: '상태 확인 중...' });
            ...
            const keyGroup = Utils.createElement('div', { className: 'form-group' });
            const keyInput = Utils.createElement('input', { className: 'form-input', type: 'text', value: Storage.get('firebaseSyncKey', ''), placeholder: '고유 동기화 키를 입력하거나 새로 생성하세요.', onchange: (e) => {
                Storage.set('firebaseSyncKey', e.target.value);
                FirebaseLogger.init(updateStatusCallback); // 키 변경 시 즉시 재연결
            }});
            
            const keyButtons = Utils.createElement('div', { className: 'form-button-group' });
            const generateButton = Utils.createElement('button', { className: 'form-button', textContent: '새 키 생성', onclick: () => {
            ...
            const copyButton = Utils.createElement('button', { className: 'form-button', textContent: '키 복사', onclick: () => {
            ...
            keyGroup.append(
                Utils.createElement('label', {className: 'form-label'}, '데이터 동기화 키'),
                Utils.createElement('p', {style: {fontSize: '12px', opacity: '0.8', margin: '0 0 5px 0'}}, '이 키를 다른 브라우저에 똑같이 입력하면 기록이 동기화됩니다. 로그인 없이 사용됩니다.'),
                keyInput,
                keyButtons
            );
        ...
        const buttonsGroup = Utils.createElement('div', { className: 'form-button-group', style: { marginTop: '15px' } });
        const viewHistoryButton = Utils.createElement('button', { className: 'form-button primary', textContent: '기록 조회', onclick: () => this._createLogViewerModal() });
        const manageConfigButton = Utils.createElement('button', { className: 'form-button', textContent: 'Firebase 설정 관리', onclick: () => this._createFirebaseConfigModal() });
    ...
    _createLogViewerModal: function() {
        ...
        const dimensions = Storage.get('imagePanelDimensions', { width: 800, height: 700 });
        const modalWidth = `${dimensions.width}px`;
        
        const contentContainer = Utils.createElement('div', { style: { padding: '15px', overflowY: 'hidden', flexGrow: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }});
        const { overlay, modal } = this._createIndependentModal('실행 기록 조회', contentContainer, modalWidth);
        modal.style.height = `${dimensions.height}px`;

        // [롤백] 필터 상태를 단일 선택 방식으로 변경
        let currentFilters = { title: 'all', qrName: 'all' };
        ...
        const initializeModal = async () => {
            ...
            const render = () => {
                ...
                // --- 필터 및 컨트롤 UI 생성 ---
                const controlsContainer = Utils.createElement('div', { style: { flexShrink: 0, marginBottom: '10px' }});
                
                const topButtons = Utils.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }});
                topButtons.append(
                    Utils.createElement('p', { style: { margin: 0 }, textContent: `총 ${allLogs.length}개의 기록` }),
                    Utils.createElement('div', { className: 'form-button-group' }, [
                        Utils.createElement('button', { className: 'form-button', textContent: 'CSV로 다운로드', onclick: () => FirebaseLogger.downloadLogsAsCsv(allLogs) }),
                        Utils.createElement('button', { className: 'form-button', textContent: '전체 기록 삭제', onclick: async () => {
                ...
                const createFilter = (label, filterType, items) => {
                    const container = Utils.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '5px', flexWrap: 'wrap', marginBottom: '5px' }});
                    container.appendChild(Utils.createElement('strong', { style: { flexShrink: 0, marginRight: '5px' } }, `${label}:`));
                ...
                // --- 테이블 UI 생성 ---
                const tableContainer = Utils.createElement('div', { style: { flexGrow: 1, overflowY: 'auto', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '4px' }});
                const tableHeader = Utils.createElement('div', { style: { display: 'flex', fontWeight: 'bold', background: 'rgba(255,255,255,0.1)', padding: '10px', borderBottom: '1px solid rgba(255,255,255,0.2)' }});
                ...
                if (filteredLogs.length === 0) {
                    tableContainer.appendChild(Utils.createElement('div', { style: { padding: '20px', textAlign: 'center' }}, '표시할 기록이 없습니다.'));
                } else {
                    filteredLogs.forEach(log => {
                        const row = Utils.createElement('div', { style: { display: 'flex', alignItems: 'center', padding: '10px', borderBottom: '1px solid rgba(255,255,255,0.1)' }});
                        
                        const qrCell = Utils.createElement('div', { style: { flex: 2, wordBreak: 'break-all', fontSize: 'clamp(11px, 1.8vw, 14px)' }, textContent: log.qrName.replace(/^DEFAULT /, '') });
                        
                        const promptCell = Utils.createElement('details', { style: { flex: 3, paddingRight: '10px' }});
                        promptCell.append(Utils.createElement('summary', { style: { cursor: 'pointer' } }, '보기'), Utils.createElement('pre', { style: { marginTop: '8px', padding: '10px', background: 'rgba(0,0,0,0.2)', borderRadius: '4px', whiteSpace: 'pre-wrap', wordBreak: 'break-all', fontSize: '12px' } }, log.prompt));
                        
                        const responseCell = Utils.createElement('div', { style: { flex: 3, paddingRight: '10px' }});
                        const responseButton = Utils.createElement('button', { className: 'form-button', style: { width: '100%', textAlign: 'left', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' } });
                        ...
                        if (log.response === '[Image Output]') {
                            ...
                            responseButton.onclick = async (e) => {
                                ...
                                } catch (err) { alert(`이미지 재생성 실패: ${err.message}`); } 
                                finally { Utils.toggleLoading(false, e.target); }
                            };
                        } else {
                        ...
                        const timeCell = Utils.createElement('div', { style: { flex: 1.5, textAlign: 'right', fontSize: '12px', opacity: 0.8, lineHeight: '1.4' }});
    ...
    _createIndependentModal: function(title, content, width = '800px') {
        ...
        const modal = Utils.createElement('div', {
            style: {
                background: 'var(--main-color)',
                width: '90%', maxWidth: width, maxHeight: '85vh',
                display: 'flex', flexDirection: 'column',
                borderRadius: '8px',
                border: '1px solid rgba(255,255,255,0.2)',
                boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.37)'
        ...
        const header = Utils.createElement('div', { className: 'settings-header', style: { padding: '0 15px', borderBottom: '1px solid rgba(255,255,255,0.1)' }});
        header.append(
            Utils.createElement('h4', { textContent: title, style: { margin: 0, flexGrow: 1 } }),
            Utils.createElement('button', { textContent: '✕', className: 'close-button', onclick: () => overlay.remove() })
        );
    ...
    _createFirebaseConfigModal: function() {
        const content = Utils.createElement('div', { style: { padding: '15px', overflowY: 'auto', flexGrow: 1 }});
        ...
        const configGroup = Utils.createElement('div', { className: 'form-group' });
        const configTextarea = Utils.createElement('textarea', { className: 'form-textarea', placeholder: 'Firebase 프로젝트 설정에서 복사한 전체 코드 스니펫을 여기에 붙여넣으세요.', textContent: Storage.getFirebaseConfig(), style: { fontFamily: 'monospace', minHeight: '150px' } });
        const saveButton = Utils.createElement('button', { className: 'form-button primary', style: { marginTop: '10px' }, textContent: '설정 저장 및 연결', onclick: () => {
            Storage.setFirebaseConfig(configTextarea.value);
            FirebaseLogger.init(updateStatusCallback);
            alert('설정이 저장되었습니다. 연결 상태를 확인하세요.');
        }});
        configGroup.append(Utils.createElement('label', {className: 'form-label'}, 'Firebase 구성 스니펫'), configTextarea, saveButton);

        const guideDetails = Utils.createElement('details', { style: { marginTop: '20px' } });
        guideDetails.append(Utils.createElement('summary', { style: { cursor: 'pointer', fontWeight: 'bold' } }, 'Firebase 설정 방법 안내'));
        const guideContent = Utils.createElement('div', { style: { marginTop: '10px', paddingLeft: '20px', borderLeft: '2px solid #555', fontSize: '14px', lineHeight: '1.6' }});
...
<pre style="padding: 10px; background: rgba(0,0,0,0.2); border-radius: 4px; white-space: pre-wrap; word-break: break-all; font-size: 12px; margin-top: 5px;">{
  "rules": {
    ...
    createImagePanel: function() {
        ...
        const imagePanel = Utils.createElement('div', { id: 'image-panel' });
        ...
        const contentWrapper = Utils.createElement('div', { id: 'image-panel-content-wrapper' });
    ...
    switchSettingsTab: function(tabId) {
        ...
        if (section) {
            let newContent;
            switch (tabId) {
                case 'settings': newContent = this.createGeneralSettingsSection(); break;
                case 'qr': newContent = this.createQrSettingsSection(); break;
                case 'ai': newContent = this.createAiSettingsSection(); break;
                case 'prompt': newContent = this.createPromptSettingsSection(); break;
                case 'lorebook': newContent = this.createLorebookSettingsSection(); break;
                case 'remote': newContent = this.createRemoteSettingsSection(); break;
                case 'logging': newContent = this.createLoggingSettingsSection(); break; // [수정] logging 케이스 추가
                default: return;
            }
            ...
            case 'prosemirror': { // 변수 범위 지정을 위한 블록 스코프
                ...
                if (lastInsertedParagraphElement) {
                    ...
                    if (finalSpan && finalSpan.firstChild) {
                        ...
                        lastInsertedParagraphElement.scrollIntoView({ behavior: 'smooth', block: 'end' });
                    }
                }
                break;
            }
...
const Features = {
    ...
    Image: {
        ...
        _renderImageContent: function(imageData, wrapper, overridePromptText = null) {
            ...
            const image = Utils.createElement('img', { className: 'generated-image', src: imageData.imageUrl, alt: '생성된 삽화' });

            const promptContainer = Utils.createElement('div', { className: 'prompt-editor-container' });
            const promptLabel = Utils.createElement('label', { className: 'form-label', textContent: '프롬프트 (수정 후 재생성 가능)' });
            const promptForEditor = overridePromptText !== null ? overridePromptText : imageData.prompt;
            const promptTextarea = Utils.createElement('textarea', { className: 'form-textarea', id: 'image-prompt-editor' }, promptForEditor || '');
    ...
    if (lastIndex < text.length) {
        ...
        _createPanelButtons: function() {
            const buttonsContainer = Utils.createElement('div', { className: 'buttons-container', style: { marginTop: '15px' } });
            const response = this.currentResponse;

            const copyButton = Utils.createElement('button', { className: 'form-button', textContent: '응답 복사' });
            copyButton.onclick = () => {
            ...
            const translateButton = Utils.createElement('button', { className: 'form-button', textContent: '번역' });
            ...
            if (typeof response === 'object' && response.imageUrl) {
                const downloadButton = Utils.createElement('button', { className: 'form-button', textContent: '다운로드' });
                ...
                const regenerateButton = Utils.createElement('button', { className: 'form-button primary', textContent: '재생성' });
